...
Designer 는 `Layout` + `PlannerOutput` + **`z_ref` (스타일 임베딩)** 을 받아 `DecorationOp[]` 을 반환하는 모델이다. 프로젝트의 **중심 R&D 산출물**. Phase 5 에 집중 (Phase 4 의 encoder 결과를 사용).

관련:
- [PLANNER.md](PLANNER.md) — 상위에서 내려오는 힌트
- [ENCODER.md](ENCODER.md) — `z_ref` 를 생산하는 self-supervised encoder
- [DATA_FORMAT.md](DATA_FORMAT.md) — 입출력 타입
- [ROADMAP.md Phase 5](ROADMAP.md#phase-5--learned-designer-mvp) — 마일스톤
...
- **입력:**
  - `Layout` (게임플레이 오브젝트 N개)
  - `PlannerOutput` (테마/밀도/세그먼트)
  - **`z_ref ∈ ℝ^256`** — encoder (Phase 4) 로부터. 참조 레벨 구간의 centroid. optional (null 가능 — CFG guidance 쪽에서 다룸).
...
2. **Nearest-neighbor (encoder)** — Phase 4 encoder 의 `z_ref` embedding 으로 학습 데이터에서 가장 비슷한 것 검색 → 그 레벨의 deco 를 레이아웃에 맞춰 warping 하여 복사. 간단하지만 강력한 베이스라인.
...
## 5.5 Encoder 기반 Conditioning (`z_ref`)

Phase 4 에서 학습된 encoder 는 레벨 구간을 `ℝ^256` 벡터로 매핑한다. Designer 는 이 벡터를 **스타일 조건** 으로 받는다.

### 5.5.1 `z_ref` 의 출처

실행 시 `z_ref` 는 3가지 방식으로 생성:

1. **Reference 레벨 + 구간** — 사용자가 UI 에서 "이런 느낌으로" 라며 특정 레벨의 x 범위를 선택. `z_ref = mean(encoder(windows_in_range))`.
2. **Planner 테마 → prototype** — Planner 가 `theme` 을 결정 → encoder 의 테마별 centroid 중 하나 선택.
3. **Null (unconditional)** — 사용자/Planner 힌트 없음. CFG 의 null branch.

### 5.5.2 Classifier-Free Guidance (CFG)

학습 시 일부 확률로 `z_ref` 를 null (0-vector 또는 learned null embedding) 로 대체:
```
L_train = E[ ||ε - ε_θ(x_t, t, c)||² ]
  where c = z_ref  with prob (1 - p_drop)
        c = ∅      with prob p_drop         (기본 p_drop = 0.1)
```

추론 시 guidance:
```
ε̂ = ε_θ(x_t, t, ∅)  +  γ · ( ε_θ(x_t, t, z_ref) - ε_θ(x_t, t, ∅) )
```
- `γ = 0` → unconditional (planner 만 반영).
- `γ = 1` → conditional (레퍼런스 그대로).
- `γ > 1` → 과장된 스타일. UX 의 "스타일 강도" 슬라이더.

UI 기본값 `γ = 1.5`. 슬라이더 범위 `[0, 3]`.

### 5.5.3 Dual conditioning

Designer 는 planner 의 `theme` / `density` / `palette` 와 encoder 의 `z_ref` 를 **둘 다** 받는다. 충돌 시 우선순위:
- `z_ref` 가 있으면 그것이 세부 스타일 담당.
- `theme` / `palette` 는 coarse control (색 팔레트는 hard constraint).
- `density` 는 둘과 독립 (밀도 = 몇 개 뿌릴지).

구현: concat 후 linear projection → condition token(s) 으로 decoder 에 주입 (cross-attention 또는 AdaLN).
